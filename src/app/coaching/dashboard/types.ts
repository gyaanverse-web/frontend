import type { SessionUser } from "@/lib/useTenantSession";
 ...
 * definition — notably one that carries **no role**, since a role only exists
 * relative to a coaching. Take the role from `useTenantSession`, which returns
 * the membership the API actually authorises against.
 */
