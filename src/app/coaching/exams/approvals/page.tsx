...
const BUCKET_KEYS: readonly string[] = ADMIN_BUCKETS.map((b) => b.key);
...
function ExamApprovalsInner() {
  const router = useRouter();
  // Owner-only hub; a teacher lands on their own exams instead.
  const { loading, user, tenant, role } = useTenantSession<Tenant>({
    allow: ["coaching_owner"],
    denyRedirect: "/coaching/exams",
  });
  ...
  // `?bucket=live` — the owner's live-monitor view is a link they can keep open
  // on a second screen, so it has to survive a reload.
  const [bucket, setBucket] = useUrlState("bucket", BUCKET_KEYS, "queue");

  // Initial load. `listLoading` starts true, so the effect only touches state in
  // the async continuation instead of synchronously in its body.
  useEffect(() => {
    if (!tenant) return;
    let cancelled = false;
    Promise.all([
      api.get<{ exams: Exam[] }>("/tenant/exams", { tenant: tenant.slug }),
      api.get<{ stats: ExamStats }>("/tenant/exams/stats", { tenant: tenant.slug }),
    ])
      .then(([data, statsData]) => {
        if (cancelled) return;
        setExams(data.exams);
    ...
    { key: "visibility", label: "Visibility", width: "14%", render: (e) => <span style={{ fontSize: 13, color: "var(--text-body)" }}>{VISIBILITY_LABEL[e.visibility]}</span> },
    {
      ...
      render: (e) => (
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <Button
            variant={e.status === "under_review" || e.status === "approved" || e.status === "live" ? "app" : "secondary"}
            size="sm"
            onClick={() => router.push(`/coaching/exams/${e.id}`)}
          >
            {e.status === "under_review" ? "Review →"
              : e.status === "approved" ? "Schedule →"
              : e.status === "live" ? "Monitor →"
              : "Open →"}
          </Button>
        </div>
      ),
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 14, marginBottom: 28 }}>
        ...
        <StatCard label="Live now" value={num(stats?.live)} sub={stats ? `${stats.scheduled} scheduled · ${stats.byStatus.approved} to schedule` : " "} />
