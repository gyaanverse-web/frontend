...
import { useUrlFlag, useUrlPage, useUrlSearch, useUrlState, useUrlWriter } from "@/lib/useUrlState";
...
// Accepted `?type=` / `?difficulty=` / `?status=` values. "" is the "All …"
// option each Select opens on, and is the value kept out of the URL.
const TYPE_FILTERS: readonly string[] = ["", ...QUESTION_TYPES.map(([k]) => k)];
const DIFFICULTY_FILTERS: readonly string[] = ["", ...DIFFICULTIES];
const STATUS_FILTERS: readonly string[] = ["", ...STATUSES];

/** `?manage=hierarchy` opens the subject-catalog drawer; absent keeps it shut. */
const MANAGE_VALUES: readonly string[] = ["", "hierarchy"];
...
function QuestionBankInner() {
  ...
  // The catalog drawer was already deep-linkable one way (`?manage=hierarchy`
  // from the dashboard's "Manage catalog" button) but never wrote the param
  // back, so closing it and reloading reopened it. Reading and writing the same
  // param makes both directions agree.
  const [manageParam, setManageParam] = useUrlState("manage", MANAGE_VALUES, "");
  const showHierarchy = manageParam === "hierarchy";
  const setShowHierarchy = (open: boolean) => setManageParam(open ? "hierarchy" : "");
  ...
  // The whole filter set is URL state: this is the screen people keep open in a
  // tab all day, and "the flagged integer questions" is worth being able to send
  // to someone. `page` moves with them — see `setFilter` below.
  const [fType] = useUrlState("type", TYPE_FILTERS, "");
  const [fDiff] = useUrlState("difficulty", DIFFICULTY_FILTERS, "");
  const [fStatus] = useUrlState("status", STATUS_FILTERS, "");
  const fVerified = useUrlFlag("verified")[0];
  const [fSearch, setFSearch] = useUrlSearch("q", { reset: ["page"] });

  // Changing a filter has to reset the page in the *same* navigation. Two
  // separate writes would leave one render on "page 7 of the new filter", which
  // for most filters is a blank table.
  const writeUrl = useUrlWriter();
  const setFilter = (patch: Record<string, string | null>) => writeUrl({ ...patch, page: null });
  ...
  // verified, search) changes, reload the list. Still debounced — `fSearch`
  // updates on the keystroke, and only its URL write is delayed. This also runs
  // the first load once `tenant` is resolved.
  //
  // Page is no longer reset here: the filter writers clear it as part of the
  // same navigation, so there is nothing left for this effect to fix.
  useEffect(() => {
    if (!tenant) return;
  ...
  function clearFilters() {
    setFSearch("");
    setFilter({ type: null, difficulty: null, status: null, verified: null, q: null });
  }
  ...
  // Both roles that reach this page may author bank content; the hook already
  // turned students away.
  const canEdit = role === "coaching_owner" || role === "teacher";
  ...
  return (
    <TeacherShell tenant={tenant} user={user} role={role} active="question-bank" headerless>
      <div style={{ position: "absolute", inset: 0, display: "flex", minHeight: 0 }}>
          ...
          <aside style={{ width: 270, flexShrink: 0, borderRight: "1px solid var(--border-light)", background: "var(--surface-card)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "8px 8px" }}>
              <HierarchyTree
                ...
                selected={selNode}
                // The hierarchy node stays component state — restoring one from
                // a URL would mean re-fetching its name to label the chip. It
                // still has to reset the page like the other filters do.
                onSelect={(n) => { setSelNode(n); setPage(1); }}
              />
            </div>
        ...
        <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          <div style={{ padding: "10px 24px", borderBottom: "1px solid var(--border-light)", background: "var(--paper-50)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", flexShrink: 0 }}>
            <Select value={fType} onChange={(e) => setFilter({ type: e.target.value })} wrapperStyle={{ width: 160 }}
              options={[{ value: "", label: "All types" }, ...QUESTION_TYPES.map(([k, l]) => ({ value: k, label: l }))]} />
            <Select value={fDiff} onChange={(e) => setFilter({ difficulty: e.target.value })} wrapperStyle={{ width: 150 }}
              options={[{ value: "", label: "All difficulty" }, ...DIFFICULTIES.map((d) => ({ value: d, label: d }))]} />
            <Select value={fStatus} onChange={(e) => setFilter({ status: e.target.value })} wrapperStyle={{ width: 140 }}
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 13, color: "var(--text-body)", cursor: "pointer", whiteSpace: "nowrap" }}>
              <input type="checkbox" checked={fVerified} onChange={(e) => setFilter({ verified: e.target.checked ? "1" : null })} style={{ accentColor: "var(--accent)", width: 15, height: 15 }} /> Verified only
            </label>
            ...
            {selNode && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "var(--accent)", background: "var(--accent-soft)", borderRadius: 999, padding: "4px 6px 4px 11px" }}>
                ...
                <button onClick={() => { setSelNode(null); setPage(1); }} title="Clear scope" aria-label="Clear hierarchy scope" style={{ border: "none", background: "none", cursor: "pointer", color: "var(--accent)", fontSize: 15, lineHeight: 1, display: "flex", alignItems: "center", padding: "0 2px" }}>×</button>
              </span>
            )}
            <div style={{ flex: 1 }} />
              ...
              <>
                ...
                {pageCount > 1 && (
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 14 }}>
                    ...
                    <Button variant="secondary" size="sm" disabled={safePage >= pageCount} onClick={() => setPage(Math.min(pageCount, safePage + 1))}>
