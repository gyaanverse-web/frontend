...
export default function ClassDetailPage() {
  ...
  const [rosterTab, setRosterTab] = useState<RosterTab>("Approved");
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      api.get<{ class: Class }>(`/tenant/classes/${classId}`, { tenant: t.slug })
        .then((data) => {
          setCls(data.class);
          setEditForm({
          ...
          if (u.role === "coaching_owner" || u.role === "teacher") {
            loadJoinCodes(t.slug, classId);
            loadStudents(t.slug, classId);
          }
          // Only the owner can reassign, so only the owner needs the teacher list.
          if (u.role === "coaching_owner") {
            api.get<{ teachers: { userId: string; name: string }[] }>("/tenant/teachers", { tenant: t.slug })
  ...
  async function copyJoinLink(jc: ClassJoinCode) {
    // Class codes redeem at /classes/join/:code (join_codes table), NOT the
    ...
    const url = `${window.location.origin}/classes/join/${jc.code}`;
    try {
  ...
  const visibleStudents = useMemo(
    () => students.filter((s) => s.status === rosterTab.toLowerCase()),
    [students, rosterTab],
  );
  ...
  const canManage = user.role === "coaching_owner" || user.role === "teacher";
    ...
    <TeacherShell tenant={tenant} user={user} active="classes">
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10, fontSize: 13 }}>
          <button onClick={() => router.push("/classes")} style={{ background: "none", border: "none", color: "var(--accent)", cursor: "pointer", padding: 0, fontFamily: "var(--font-body)", fontSize: 13 }}>My Classes</button>
          ...
          <div className="gv-card" style={{ overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border-light)", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              ...
              <Tabs
                ...
                onChange={(label) => setRosterTab(label.split(" ")[0] as RosterTab)}
              />
            </div>
            {studentsError ? (
              ...
              <DataTable columns={columns} rows={visibleStudents} maxHeight={440} style={{ border: "none", borderRadius: 0 }} />
            )}
          </div>

          {/* Join codes */}
          <div className="gv-card" style={{ overflow: "hidden" }}>
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border-light)", display: "flex", alignItems: "center" }}>
              <h4 style={{ margin: 0, flex: 1, fontSize: 15 }}>Join codes</h4>
              <Button variant="app" size="sm" onClick={() => setShowJcForm((v) => !v)}>Generate</Button>
            </div>

            {showJcForm && (
              <form onSubmit={handleCreateJoinCode} style={{ padding: "14px 18px", borderBottom: "1px solid var(--border-light)", background: "var(--paper-50)", display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {joinCodes.length === 0 && !jcError ? (
                ...
                joinCodes.map((jc) => {
                  ...
                  return (
                    <div key={jc.id} style={{ padding: "12px 14px", border: "1px solid var(--border-light)", borderRadius: 10, background: "var(--paper-50)" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                        <code style={{ fontFamily: "var(--font-mono)", fontSize: 13.5, fontWeight: 700, color: "var(--text-heading)", letterSpacing: "0.05em", flex: 1 }}>{jc.code}</code>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <Button variant="ghost" size="sm" onClick={() => copyJoinLink(jc)}>
                          {copiedJcId === jc.id ? "Copied!" : "Copy link"}
                        </Button>
                        {confirmRevokeJcId === jc.id ? (
                          <>
                            <Button variant="danger" size="sm" onClick={() => handleRevokeJoinCode(jc.id)}>Confirm</Button>
                            <Button variant="ghost" size="sm" onClick={() => setConfirmRevokeJcId(null)}>Cancel</Button>
                          </>
                        ) : (
                          <Button variant="ghost" size="sm" style={{ color: "var(--danger)" }} onClick={() => setConfirmRevokeJcId(jc.id)}>Revoke</Button>
                        )}
                      </div>
                    </div>
                  );
      ...
      {/* Settings modal */}
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Class settings" width={460}>
