...
import type { Class, Tenant, User } from "../dashboard/types";
...
export default function ClassesPage() {
  ...
  async function loadClasses(slug: string) {
    setListLoading(true); setPageError("");
    try {
      const data = await api.get<{ classes: Class[] }>("/tenant/classes", { tenant: slug });
      setClasses(data.classes);
    } catch (err) {
      setPageError(err instanceof Error ? err.message : "Failed to load classes");
    } finally {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      loadClasses(tr.value.tenant.slug);
      setLoading(false);
    });
  }, [router]);
      ...
      {totalPending > 0 && (
        <div
          style={{
            ...
            background: "#fef9c3", border: "1px solid #fde068", color: "#713f12",
          }}
        >
          ...
          <Link
            href={`/classes/${pendingClasses[0].id}`}
            className="gv-btn gv-btn--sm"
            style={{ textDecoration: "none", background: "#713f12", color: "#fff", border: "1px solid #713f12", whiteSpace: "nowrap" }}
          >
            ...
            >
              ...
              <div style={{ marginTop: "auto", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", paddingTop: 12, borderTop: "1px solid var(--border-light)" }}>
                ...
                {(cls.pendingCount ?? 0) > 0 && (
                  <span
                    style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 700, padding: "3px 10px", borderRadius: "var(--radius-pill)", background: "#fef9c3", color: "#713f12", border: "1px solid #fde068" }}
                  >
